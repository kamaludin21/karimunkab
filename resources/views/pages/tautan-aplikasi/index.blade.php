@php
  $linkCategory = \App\Models\LinkCategory::get();
  $link = \App\Models\Link::get();
@endphp
    ...
    <div class="flex flex-col md:flex-row gap-2 py-16">
      <div class="w-full md:w-1/3 p-4 h-min border rounded-lg border-slate-200 space-y-2">
        ...
        <ul class="space-y-2 text-slate-500">
          ...
          <li class="hover:bg-slate-200 hover:px-2 py-2 duration-200 rounded-md">Semua</li>
          @foreach ($linkCategory as $item)
            <li class="hover:bg-slate-200 hover:px-2 py-2 duration-200 rounded-md">{{ $item->title }}</li>
          @endforeach
        </ul>
      </div>
      <div class="w-full md:w-2/3 p-2 md:p-4 border rounded-lg border-slate-200 space-y-4">
        ...
        @forelse ($link as $item)
          <div class="bg-white border border-slate-200 rounded-lg flex p-2 md:p-4 items-center justify-between">
            <div>
              <p class="text-lg text-slate-800 font-medium">{{ $item->description }}</p>
              <a href="{{ $item->url }}" class="text-slate-600">{{ $item->url }}</a>
            </div>
            <div class="flex gap-1 items-center">
              {{-- <button class="hover:bg-slate-200 bg-white p-2 rounded active:scale-95">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round"
                  stroke-linejoin="round" class="h-5 w-5 stroke-2 text-slate-600">
              ...
              <a href="{{ $item->url }}" target="_blank" class="hover:bg-slate-200 bg-white p-2 rounded active:scale-95">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round"
                  stroke-linejoin="round" class="h-5 w-5 stroke-2 text-slate-600">
                  <path stroke="none" d="M0 0h24v24H0z" fill="none" />
