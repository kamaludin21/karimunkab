  ...
  <div class="max-w-screen-lg dot-pattern mx-auto grid gap-6 rounded-none md:rounded-xl pt-16 pb-6 px-2 md:px-4">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-4 mt-4 md:mt-8">
      @foreach ($phones as $phone)
        <div class="p-2 md:p-4 rounded-lg bg-white h-full flex flex-col justify-between">
          <div class="space-y-1">
            <p class="text-lg font-medium text-slate-800 leading-5 line-clamp-2">{{ $phone->service_name }}</p>
            <p class="text-base font-light text-slate-600 line-clamp-2">{{ $phone->contact_name }}</p>
          </div>
          <div class="flex flex-col md:flex-row gap-2 mt-4 font-medium">
            <a href="tel:{{ $phone->phone_number }}"
              class="text-center text-slate-700 gap-1 border border-slate-400 py-1.5 md:py-2 px-2 flex-1 rounded-lg active:scale-95">
              Telepon
            </a>
            {{-- if is_whatsapp true --}}
            @if ($phone->is_whatsapp)
              <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $phone->phone_number) }}" target="_blank"
                class="flex items-center justify-center gap-1 bg-emerald-700 text-white py-1.5 px-2 md:py-2 flex-1 rounded-lg active:scale-95">
                <span>Whatsapp</span>
    ...
    <div
      ...
      <a href="/nomor-penting"
        class="text-slate-300 hover:text-white hover:bg-slate-600 cursor-pointer border border-slate-300 w-fit mx-auto px-3 py-1 rounded-full flex active:scale-95 gap-1 items-center">
