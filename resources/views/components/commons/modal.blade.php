...
<div x-data x-init="@if ($show) $store.modal.open('{{ $id }}') @endif" x-show="$store.modal.isOpen('{{ $id }}')"
  @keydown.escape.window="$store.modal.close()" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50"
  x-cloak>
  <div class="bg-white rounded-lg shadow-lg max-w-[calc(100%-3rem)] w-full max-w-fit md:max-w-lg max-h-dvh relative"
