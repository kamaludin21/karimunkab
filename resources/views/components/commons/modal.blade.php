...
<div x-data x-init="@if ($show) $store.modal.open('{{ $id }}') @endif" x-show="$store.modal.isOpen('{{ $id }}')"
  ...
  <div class="bg-white rounded-lg shadow-lg w-full max-w-md relative" @click.away="$store.modal.close()">
    <div class="absolute -right-4 -top-4">
      <button @click="$store.modal.close()" type="button" class="shadow-md bg-red-500 border-2 border-white rounded-full p-1">
