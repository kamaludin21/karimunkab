...
<ul class="py-4 px-2 grid divide-y divide-slate-200 divide-dashed">
  @forelse ($documents as $doc)
    <div class="flex items-center justify-between hover:bg-slate-50 py-3 px-2">
      <div class="py-2 w-full">
        ...
        <a href="/publikasi-dokumen/{{ $doc->slug }}" class="text-lg font-medium hover:underline text-slate-600 line-clamp-1 w-fit">
          {{ $doc->title }}
        </a>
      </div>
      <div x-data="{ open: false }" class="flex w-fit items-center justify-end gap-2 px-2 text-slate-600">
        <div class="relative inline-flex w-fit text-sm text-slate-500">
          ...
          <div x-show="open" x-cloak @click="open = false" class="fixed inset-0 z-30 bg-slate-900/20"></div>
          ...
          <div x-show="open" x-cloak @click.outside="open = false"
            ...
            class="absolute top-full right-0 z-40 mt-1 w-fit rounded-md bg-white ring-1 ring-slate-200 shadow-lg">
